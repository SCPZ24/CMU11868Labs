...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_1_2
        ...
        grad_inp = out_grad.f.attn_softmax_bw(out_grad, ctx.saved_values[0])
        grad_mask = zeros(ctx.saved_values[1].shape, backend=out_grad.backend)
...
class LayerNorm(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_2
        ...
        inp_grad, gamma_grad, beta_grad = inp.f.layernorm_bw(out_grad, inp, gamma, beta, vars, means)
